...
## Свойство Options
  ...
  - **mode** - С дополнительным значением. Используемый режим работы модуля. Подробнее про типы режимов [описано ниже](#Режимы-работы-tango-модуля)
...
Танго модуль запускается в одном из перечисленных режимов:
  ...
  - **SERVNCLIENT_ALL_RO** - Серверное и клиентное управление выводом информации, считываемых с атрибутов и pipe. А также запуск команд в tango-модуле (или группе), прописанном в Property; option:  `mode=ser_cli_all_ro`.
  - **SERVNCLIENT_ALL** -  Серверное и клиентное управление выводом информации, считываемых с атрибутов и pipe. Запуск команд в tango-модуле (или группе), прописанном в Property. Также запуск команд и чтение атрибутов с любых танго-модулей; option: `mode=ser_cli_all`
  - **SERVNCLIENT_ALIAS_RO** - Серверное и клиентное управление выводом информации, считываемых с атрибутов и pipe. Запуск команд в tango-модуле (или группе), прописанном в Property. Запрещён запуск команд в модулях указанном клиентом. Чтение атрибутов только с танго-модулей, имеющих alias; option: `mode=ser_cli_ali_ro`
  - **SERVNCLIENT_ALIAS** - Серверное и клиентное управление выводом информации, считываемых с атрибутов и pipe. Запуск команд в tango-модуле (или группе), прописанном в Property. Запрещён запуск команд в модулях указанном клиентом. Чтение атрибутов, а также запуск команд только с танго-модулей, имеющих alias; option: `mode=ser_cli_ali`
  - **CLIENT_ALL_RO** - Клиентное управление выводом информации, считываемых с атрибутов и pipe. При использовании данного режима, из сервера приходят только данные, определённые пользователем. Также запрещён запуск команд; option: `mode=cli_all_ro`
  - **CLIENT_ALL** - Клиентное управление выводом информации, считываемых с атрибутов и pipe. При использовании данного режима, из сервера приходят только данные, определённые пользователем. Возможен запуск команд на любых танго-модулях; option: `mode=cli_all`
  - **CLIENT_ALIAS_RO** - Клиентное управление выводом информации, считываемых с атрибутов и pipe только с танго-модулей, имеющих alias. При использовании данного режима, из сервера приходят только данные, определённые пользователем.; option: `mode=cli_ali_ro`
  - **CLIENT_ALIAS** - Клиентное управление выводом информации, считываемых с атрибутов и pipe,  только с танго-модулей, имеющих alias. При использовании данного режима, из сервера приходят только данные, определённые пользователем. Также возможен запуск команд только с танго-модулей, имеющих alias; option: `mode=cli_ali`

Для включение режима, отличного от SERVER, нужно добавить в Property `Options` значение, заданнoе для режима. ([подробнее про формат опций здесь](#Свойство-options)). Формат: `mode=выбранный_режим`.
...
- Также сервер должен содержать метод `check_permissions(const Tango::DevVarStringArray (*argin)`, применяемый для аутентификации, в который передаётся массив, содержащий перечисленные ниже данные. Данный метод должен возвращать true или false
...
Перечень команд (`"type_req": "команда"`):
 ...
 - **timer_change** - Изменения периода обновления данных. Для изменения, должны быть также переданы значение таймера в милисекундах `"msec": value` (int) и объект `devices`. Минимальное значение 1000 мсек
...
В режимах, отличных от `SERVER` возможны чтение данных с атрибутов и pipe, а также запуск команд на танго модулях (кроме ReadOnly режимов). Перечень возможных режимов с кратким описанием приведены [здесь](#Режимы-работы-tango-модуля)
...
Для выставления необходимых параметров для конкретных атрибутов из pipe, к массиву `pipes` следует добавить перечень атрибутов с необходимыми параметрами.
