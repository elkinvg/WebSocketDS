...
- В данные из событий также добавлены атрибуты "device_name" и "attr_short", берущиеся из парсинга полного имени.
- Добавлен класс MyAttributeData, используемый для извлечения данных из Tango::AttributeData. Используется в обоих режимах
- В MyEventData из (1.0.11) переделан в класс, добавлен указатель на MyAttributeData. Также используется в обоих режимах
- Добавлена опция `jsonout`, запрещающая заключать в кавычки string ответ
- Добавлена структура для пользовательских опций
...
- Удалена переменная cache из WSThread\_\*, в которой хранилось последнее сообщение для клиентов. При использовании подписки на события в серверном режиме вызывало системную ошибку "corrupted double-linked"
